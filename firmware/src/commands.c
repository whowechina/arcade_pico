...
static void disp_hall()
{
    ...
    for (int i = 0; i < hebtn_keynum(); i++) {
        ...
        printf("  Key %2d: %4d->%4d, On: %2d, Off: %2d.\n",
               i + 1, arcade_cfg->calibrated.up[i], arcade_cfg->calibrated.down[i],
                arcade_cfg->trigger.on[i] + 1, arcade_cfg->trigger.off[i] + 1);
    }
}
...
static void handle_trigger(int argc, char *argv[])
{
    ...
    for (int i = 0; i < hebtn_keynum(); i++) {
        if (all_key || (i == key)) {
            arcade_cfg->trigger.on[i] = on;
            arcade_cfg->trigger.off[i] = off;
        }
    }
    config_changed();
