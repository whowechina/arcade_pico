...
#define ARCADE_PICO_REPORT_DESC_JOYSTICK                                 \
    ...
    HID_COLLECTION(HID_COLLECTION_APPLICATION),                          \
        ...
        HID_REPORT_COUNT(8), HID_REPORT_SIZE(1),  /* Padding */          \
        HID_INPUT(HID_CONSTANT | HID_VARIABLE | HID_ABSOLUTE),           \
    HID_COLLECTION_END

#define ARCADE_PICO_REPORT_DESC_LIGHTS                                   \
  HID_USAGE_PAGE(HID_USAGE_PAGE_DESKTOP),                                \
  HID_USAGE(0x00),                                                       \
  HID_COLLECTION(HID_COLLECTION_APPLICATION),                            \
      HID_REPORT_ID(REPORT_ID_LIGHTS)                                    \
      HID_REPORT_COUNT(24), HID_REPORT_SIZE(8),                          \
      HID_LOGICAL_MIN(0x00), HID_LOGICAL_MAX_N(0x00ff, 2),               \
      HID_USAGE_PAGE(HID_USAGE_PAGE_ORDINAL),                            \
      HID_STRING_MINIMUM(6), HID_STRING_MAXIMUM(29),                     \
